...
void drawImageDMA(int row, int col, int width, int height, const u16 *image)
{
  for (int i = 0; i < height; i++)
  {
    DMA[3].src = image + (i * width);                     // starting row in image
    DMA[3].dst = videoBuffer + OFFSET(row + i, col, WIDTH); // starting row in video buffer
    /*
     * control
     * width - number of pixels to transfer
     * DMA_ON - enable DMA
     * DMA_SOURCE_INCREMENT - increment DMA to next source 
     * DMA_DESTINATION_INCREMENT - increment DMA to next destination
     */
...
{
  for (int i = 0; i < height; i++) {  
      DMA[3].src = image + ((row + i) * WIDTH + col);     // starting row of the image
      DMA[3].dst = videoBuffer + ((row + i) * WIDTH + col); // destination
